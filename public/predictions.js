...
function confidenceBlurb(row){
  ...
  return Number.isFinite(value)?`<span class="forecast-signal model-confidence muted" title="Based on the weaker fighter's pre-fight sample strength">Model confidence: ${Math.round(value)}%</span>`:'';
}
function fanConsensusBlurb(row){
  const total=Number(row.fan_total||0),href=`${fightLink(row)}#fans-vs-model`;
  if(!total)return `<a class="forecast-signal fan-consensus muted" href="${href}">Fan consensus: Be the first to pick →</a>`;
  const picks=`${total.toLocaleString()} pick${total===1?'':'s'}`;
  if(!row.fan_consensus_pick)return `<a class="forecast-signal fan-consensus muted" href="${href}">Fans: 50/50 · ${picks} →</a>`;
  const side=row.fan_consensus_pick,name=side==='a'?row.fighter_a_name:row.fighter_b_name,p=side==='a'?row.fan_a_pct:row.fan_b_pct;
  return `<a class="forecast-signal fan-consensus muted" href="${href}">Fans: ${escapeHtml(name)} ${percent(p)} · ${picks} →</a>`;
}
async function loadForecasts(){
  ...
  document.querySelector('#event-cards').innerHTML=[...events.values()].sort((a,b)=>a[0].starts_at.localeCompare(b[0].starts_at)).map(rows=>{
    const event=rows[0];
    return `<article class="event-card"><header><div><div class="eyebrow">${escapeHtml(event.event_date)}${event.event_live?' · FIGHT DAY':''}</div><h2>${escapeHtml(event.event_name)}</h2><span class="muted">${rows.length} saved predictions · Starts ${escapeHtml(savedTime(event.starts_at))}</span></div><a href="${escapeHtml(event.source_url)}" target="_blank" rel="noopener">Official card ↗</a></header>${rows.map(r=>`<div class="forecast-bout"><div class="forecast-matchup">${fighter(r,'a')}<span class="forecast-versus">VS</span>${fighter(r,'b')}</div><div class="probability-bar" aria-hidden="true"><span style="width:${Number(r.fighter_a_probability)*100}%"></span></div><p class="result-label grade-${escapeHtml(r.grade)}">${escapeHtml(resultLabel(r))}</p><div class="forecast-signals">${confidenceBlurb(r)}${fanConsensusBlurb(r)}</div><p><a class="fight-detail-link" href="${fightLink(r)}">Fight details, stats & reasoning →</a></p><div class="forecast-meta"><span>${escapeHtml(r.weight_class)}</span><span>Locked ${escapeHtml(savedTime(r.locked_at))}</span></div></div>`).join('')}</article>`;
