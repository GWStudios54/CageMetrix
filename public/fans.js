(() => {
  ...
  function renderPrediction(){
    ...
    const modelLabel=m.model_version&&m.model_version!=='unknown'?`CageMetrix Predictor ${esc(m.model_version)}`:'CageMetrix Predictor';
    root.innerHTML=`<div class="fans-compare"><article class="fans-side"><h3>${modelLabel}</h3><div class="fans-line"><span>${esc(b.fighter_a_name)}</span><strong>${pct(m.fighter_a_probability)}</strong></div><div class="fans-line"><span>${esc(b.fighter_b_name)}</span><strong>${pct(m.fighter_b_probability)}</strong></div>${b.status==='completed'?modelGrade:''}</article><article class="fans-side"><h3>Fan Consensus</h3>${fanLines}<p class="fans-meta">${f.total?`${f.total.toLocaleString()} fan pick${f.total===1?'':'s'}`:'Be the first fan to pick this fight.'}</p>${b.status==='completed'?fanGrade:''}</article></div>${f.open?`<div class="fan-pick-actions"><button class="fan-pick" data-pick="a" aria-pressed="${f.my_pick==='a'}">Pick ${esc(b.fighter_a_name)}</button><button class="fan-pick" data-pick="b" aria-pressed="${f.my_pick==='b'}">Pick ${esc(b.fighter_b_name)}</button></div><p class="fans-meta">Your pick may be changed until the card starts. Then every fan vote is locked for the permanent Fans vs. Model record.</p>`:`<p class="fans-meta">Fan predictions are locked. ${esc(f.lock_policy)}</p>`}<p id="fan-prediction-feedback" class="fan-feedback" role="status"></p>`;
  }
  function scoreOptions(value='',aName='Fighter A',bName='Fighter B'){
    const options=[['','Choose a score'],['10-9',`${aName} 10–9`],['9-10',`${bName} 10–9`],['10-8',`${aName} 10–8`],['8-10',`${bName} 10–8`],['10-7',`${aName} 10–7`],['7-10',`${bName} 10–7`],['10-10','10–10']];
    return options.map(([v,label])=>`<option value="${v}" ${v===value?'selected':''}>${esc(label)}</option>`).join('');
  }
  function renderScorecards(){
    ...
    const deadline=s.closes_at?`Submissions close ${date(s.closes_at)} — 48 hours after the card finished.`:'This fight can be scored now. The 48-hour clock begins when the full card finishes.';
  ...
  async function put(path,body){
    ...
    const response=await r.json();if(!r.ok)throw new Error(response.error||'Could not save.');data=response;return response;
  }
  document.addEventListener('change',event=>{if(event.target.closest?.('#fan-score-form'))scoreDirty=true;});
  document.addEventListener('click',async event=>{
    ...
    try{await put('fan-prediction',{pick:button.dataset.pick});renderPrediction();const feedback=$('#fan-prediction-feedback');if(feedback)feedback.textContent='Fan pick saved.';}catch(error){const feedback=$('#fan-prediction-feedback');if(feedback)feedback.textContent=error.message||'Could not save your pick.';}finally{saving=false;document.querySelectorAll('.fan-pick').forEach(b=>b.disabled=false);}
  });
  document.addEventListener('submit',async event=>{
    ...
    try{await put('fan-scorecard',{rounds});scoreDirty=false;render();const feedback=$('#fan-score-feedback');if(feedback)feedback.textContent='Fan scorecard saved.';}catch(error){const feedback=$('#fan-score-feedback');if(feedback)feedback.textContent=error.message||'Could not save your scorecard.';}finally{saving=false;if(button)button.disabled=false;}
  });
