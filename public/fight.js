(() => {
  ...
  function renderSnapshot(){
    ...
    $('#snapshot-download').href=`/api/fights/${id}/snapshot?prediction=${p.id}`;
    if(!s?.available){
      $('#snapshot-note').textContent=s?.reason||'The original feature snapshot is unavailable. Current ratings are not substituted for pre-fight inputs.';
      $('#fight-stats').innerHTML='<p class="warning">Pre-fight comparison unavailable for this archived prediction.</p>';
      $('#fight-drivers').innerHTML=p.saved_drivers?.length?`<p class="muted">These are the driver contributions saved with the original prediction; the full input archive is unavailable.</p>${driverHtml(p.saved_drivers,f)}`:'<p class="muted">No verified feature explanation was retained for this prediction.</p>';
      $('#feature-table').textContent='Exact features unavailable.';
    }else{
      $('#snapshot-note').innerHTML=`${s.provenance==='verified_archive'?'Recovered from the matching rating archive and verified against the saved prediction.':'Captured with the original prediction.'} Data through ${esc(s.source_max_date)} · CMR ${esc(s.cmr_version)}.<br><span class="snapshot-key">${esc(s.input_snapshot_key)}</span>`;
      $('#fight-stats').innerHTML=`<table class="comparison-table"><caption class="sr-only">Preserved pre-fight statistics, compared side by side</caption><thead><tr><th scope="col">CageMetrix metric</th><th scope="col">${esc(f.a.name)}</th><th scope="col">${esc(f.b.name)}</th></tr></thead><tbody>${fields.map(([key,label])=>`<tr class="${key==='cmr'?'cmr-row':''}"><th scope="row">${label}</th>${['a','b'].map(side=>`<td>${num(f[side].rating?.[key],key==='bouts'?0:1)}</td>`).join('')}</tr>`).join('')}</tbody></table><p class="muted">Ratings are on a 0–100 scale except Elo, bout count and minutes. Sample strength measures evidence, not win chance.${!f.a.rating||!f.b.rating?' An unrated fighter has no CMR statistics; dashes are intentional.':''}</p>`;
      const fallback=s.model_used==='elo_fallback'||s.model_used==='elo_baseline';
      $('#fight-drivers').innerHTML=`<p class="muted">${fallback?(s.model_used==='elo_fallback'?'This matchup uses Predictor 0.1’s neutral-start Elo fallback because at least one fighter was unrated. Only the Elo difference drives this probability; an unrated fighter starts at 1500.':'This earlier prediction used the archived Elo baseline. Only Elo drove its probability.'):'These are the actual grouped contributions to Predictor 0.1. An edge means that group pushed the model toward that fighter; it is not a forecast of a specific technique or finish.'}</p>${s.drivers.length?driverHtml(s.drivers,f):'<p>The model’s measured inputs produce an even matchup.</p>'}`;
      $('#feature-table').innerHTML=`<div class="feature-scroll" tabindex="0" role="region" aria-label="Exact frozen model inputs"><table class="feature-table"><thead><tr><th>Feature</th><th>Value</th><th>Coefficient</th><th>Contribution</th></tr></thead><tbody>${s.features.map(x=>`<tr><th scope="row">${esc(x.name)}</th><td>${esc(x.value)}</td><td>${esc(x.coefficient)}</td><td>${esc(x.contribution)}</td></tr>`).join('')}</tbody></table></div><p class="muted">Download the inputs for the full-precision ratings, model parameters and feature values.</p>`;
    }
    const archive=payload.predictions.filter(x=>x.id!==p.id);
    $('#prediction-archive').hidden=!archive.length;
    $('#prediction-archive').innerHTML=`<h2>Other saved predictions for this fight</h2><p>Each model’s original probability and inputs remain separate.</p>${archive.map(x=>`<a href="/fights/${id}?prediction=${x.id}">${esc(x.model_name)} ${esc(x.model_version)} · ${pct(x.fighter_a_probability)} / ${pct(x.fighter_b_probability)} →</a>`).join('')}`;
  }
  function driverHtml(drivers,f){
    const max=Math.max(...drivers.map(d=>Math.abs(d.contribution)),0.001);
    return drivers.map(d=>`<article class="driver"><div class="driver-heading"><strong>${esc(d.label)}</strong><span>${esc(f[d.side]?.name||'')} edge</span></div><div class="driver-meter" aria-hidden="true"><span style="width:${Math.abs(d.contribution)/max*48}%;${d.side==='a'?'right:50%;background:#e43b34':'left:50%;background:#93bce8'}"></span></div><p>${esc(f[d.side]?.name||'')} receives a ${num(Math.abs(d.contribution),3)} log-odds contribution from this group.</p></article>`).join('');
  }
  function renderResult(){
    ...
    else if(b.status==='cancelled')$('#fight-result').innerHTML='<div class="replacement-note">This matchup was cancelled. Its original prediction and pre-fight inputs remain available and are excluded from accuracy.</div>';
