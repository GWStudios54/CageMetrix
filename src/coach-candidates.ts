...
export async function coachCandidatesAdminApi(request:Request,env:Env){
  ...
  if(request.method==='GET'){
    ...
    const rows=await env.DB.prepare(`SELECT c.id,c.candidate_key,c.source_url,c.source_title,c.publisher,c.published_at,c.source_type,c.fighter_name,c.normalized_name,c.source_key,c.source_fighter_id,c.detected_coach_name,c.detected_event_type,c.detected_summary,c.extraction_method,c.review_status,c.discovered_at,c.reviewed_at,c.notes,p.profile_slug
    ...
    return json({data:rows.results||[],meta:{status,limit,count:rows.results?.length||0,policy:'Discovery candidates are private review leads, not published coach facts about any fighter.'}});
  }
  ...
  const candidate=await env.DB.prepare(`SELECT id,review_status FROM coach_intel_candidates WHERE id=? LIMIT 1`).bind(id).first<Row>();if(!candidate)return json({error:'candidate_not_found'},404);
  await env.DB.prepare(`UPDATE coach_intel_candidates SET review_status=?,reviewed_at=CURRENT_TIMESTAMP,notes=? WHERE id=?`).bind(status,String(value.notes||'').trim().slice(0,2000)||null,id).run();
