...
function baseMeta(rewriter:HTMLRewriter,title:string,description:string,canonical:string){return rewriter.on('title',{element(el){el.setInnerContent(title);}}).on('meta[name="description"]',{element(el){el.setAttribute('content',description);}}).on('meta[property="og:title"]',{element(el){el.setAttribute('content',title);}}).on('meta[property="og:description"]',{element(el){el.setAttribute('content',description);}}).on('meta[property="og:url"]',{element(el){el.setAttribute('content',canonical);}}).on('meta[name="twitter:title"]',{element(el){el.setAttribute('content',title);}}).on('meta[name="twitter:description"]',{element(el){el.setAttribute('content',description);}});}
...
export async function homePage(request:Request,env:Env){
  ...
  const title='UFC Rankings, Fight Predictions & MMA Analytics | CageMetrix™',description='Opponent-adjusted UFC rankings, fighter stats, locked fight predictions, community picks and fight-night discussion. Compare CMR™, technical ratings and model win probabilities.',canonical=`${SITE}/`,schema={'@context':'https://schema.org','@graph':[{'@type':'WebSite',name:'CageMetrix',url:canonical,description},{'@type':'Organization',name:'CageMetrix',url:canonical,logo:`${SITE}/logo.svg`} ]};
  const block=dashboard(event,rows,threads);
  const rewriter=baseMeta(new HTMLRewriter(),title,description,canonical).on('.hero',{element(el){if(block)el.after(block,{html:true});}}).on('.topbar nav a[href="/community"]',{element(el){el.setAttribute('href','/forum');el.setInnerContent('Forum');}}).on('head',{element(el){el.append(`<link rel="stylesheet" href="/home-dashboard.css"><script src="/home-dashboard.js" defer></script><link rel="canonical" href="${canonical}"><meta name="robots" content="index,follow,max-image-preview:large"><script type="application/ld+json">${jsonLd(schema)}</script>`,{html:true});}});
  const transformed=rewriter.transform(response),out=new Response(transformed.body,transformed);out.headers.set('cache-control','public, max-age=30, s-maxage=90');return out;
}

export async function predictionsPage(request:Request,env:Env){
  const [response,event]=await Promise.all([asset(request,env,'/predictions.html'),nextEvent(env)]),title='UFC Fight Predictions, Picks & Win Probabilities | CageMetrix™',description='Upcoming UFC fight predictions with locked CageMetrix™ win probabilities, matchup stats and an auditable record of every model pick.',canonical=`${SITE}/predictions.html`,schema={'@context':'https://schema.org','@type':'CollectionPage',name:'CageMetrix UFC Fight Predictions',url:canonical,description,isPartOf:{'@type':'WebSite',name:'CageMetrix',url:`${SITE}/`}};
  ...
  const transformed=rewriter.transform(response),out=new Response(transformed.body,transformed);out.headers.set('cache-control','public, max-age=30, s-maxage=120');return out;
}
