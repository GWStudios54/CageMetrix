...
const xmlEscape=(value:unknown)=>escape(value);
const jsonLd=(value:unknown)=>JSON.stringify(value).replace(/</g,'\\u003c');
const pct=(value:unknown)=>`${(Number(value||0)*100).toFixed(1)}%`;
const isoDate=(value:unknown)=>/^\d{4}-\d{2}-\d{2}/.test(String(value||''))?String(value).slice(0,10):null;
const prettyDate=(value:unknown)=>{const d=isoDate(value);if(!d)return '';return new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric',year:'numeric',timeZone:'UTC'}).format(new Date(`${d}T12:00:00Z`));};

export function eventSeoName(event:Row){
  if(event.slug==='ufc-fight-night-september-05-2026')return 'UFC Paris';
  const raw=String(event.name||'UFC event').replace(/\s*:\s*[^:]+\s+vs\s+[^:]+$/i,'').trim();
  return raw||'UFC event';
}
function eventSchemaStatus(status:unknown){
  if(status==='completed')return 'https://schema.org/EventCompleted';
  if(status==='cancelled')return 'https://schema.org/EventCancelled';
  return 'https://schema.org/EventScheduled';
}
function locationSchema(event:Row){
...
export async function sitemap(env:Env){
  ...
  for(const row of events.results||[])urls.push([`${SITE}/events/${encodeURIComponent(String(row.slug))}`,isoDate(row.event_date),'daily','0.9']);
  for(const row of fights.results||[])urls.push([`${SITE}/fights/${row.id}`,isoDate(row.event_date),'daily','0.8']);
  for(const row of fighters.results||[])urls.push([`${SITE}/fighters/${encodeURIComponent(String(row.slug))}`,isoDate(row.lastmod),'weekly','0.7']);
  const body=`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls.map(([loc,lastmod,changefreq,priority])=>`  <url><loc>${xmlEscape(loc)}</loc>${lastmod?`<lastmod>${lastmod}</lastmod>`:''}<changefreq>${changefreq}</changefreq><priority>${priority}</priority></url>`).join('\n')}\n</urlset>\n`;
...
export async function eventPage(request:Request,env:Env,slug:string){
  ...
  const rows=await env.DB.prepare(`
    ...
    SELECT b.id,b.bout_order,b.weight_class,b.scheduled_rounds,b.status,b.winner_id,
      a.id fighter_a_id,a.name fighter_a_name,a.slug fighter_a_slug,
      z.id fighter_b_id,z.name fighter_b_name,z.slug fighter_b_slug,
    ...
    LEFT JOIN picks p ON p.bout_id=b.id AND p.rn=1
    WHERE b.event_id=? ORDER BY b.bout_order ASC,b.id ASC`).bind(PREDICTOR_NAME,event.id).all<Row>();
  const bouts=rows.results||[];
  const main=bouts[0];
  const label=eventSeoName(event);
  ...
  const schema:any={
    '@context':'https://schema.org','@type':'SportsEvent',name:`${label}: ${matchup}`,url:canonical,
    startDate:event.starts_at||event.event_date,eventStatus:eventSchemaStatus(event.status),sport:'Mixed Martial Arts',
    organizer:{'@type':'Organization',name:'UFC',url:'https://www.ufc.com/'},
    ...(event.source_url?{sameAs:event.source_url}:{}),...(locationSchema(event)?{location:locationSchema(event)}:{}),
    competitor:main?[{'@type':'Person',name:main.fighter_a_name,url:`${SITE}/fighters/${main.fighter_a_slug}`},{'@type':'Person',name:main.fighter_b_name,url:`${SITE}/fighters/${main.fighter_b_slug}`}]:undefined,
    subEvent:bouts.map((b:Row)=>({'@type':'SportsEvent',name:`${b.fighter_a_name} vs ${b.fighter_b_name}`,url:`${SITE}/fights/${b.id}`,startDate:event.starts_at||event.event_date,sport:'Mixed Martial Arts',competitor:[{'@type':'Person',name:b.fighter_a_name,url:`${SITE}/fighters/${b.fighter_a_slug}`},{'@type':'Person',name:b.fighter_b_name,url:`${SITE}/fighters/${b.fighter_b_slug}`}]}))
  };
  const breadcrumbs={'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[
    {'@type':'ListItem',position:1,name:'CageMetrix',item:`${SITE}/`},
    {'@type':'ListItem',position:2,name:'UFC Predictions',item:`${SITE}/predictions.html`},
    {'@type':'ListItem',position:3,name:label,item:canonical}
  ]};
  const cards=bouts.map((b:Row)=>{
    const hasPick=Number.isFinite(Number(b.fighter_a_probability))&&Number.isFinite(Number(b.fighter_b_probability));
    const aPicked=hasPick&&Number(b.picked_fighter_id)===Number(b.fighter_a_id),bPicked=hasPick&&Number(b.picked_fighter_id)===Number(b.fighter_b_id);
    return `<article class="event-fight"><p class="eyebrow">${escape(b.weight_class||'UFC bout')}</p><h2><a href="/fights/${b.id}">${escape(b.fighter_a_name)} vs ${escape(b.fighter_b_name)} prediction</a></h2><p><a href="/fighters/${escape(b.fighter_a_slug)}">${escape(b.fighter_a_name)}</a> ${hasPick?`<strong>${pct(b.fighter_a_probability)}${aPicked?' model pick':''}</strong>`:''} <span aria-hidden="true">·</span> <a href="/fighters/${escape(b.fighter_b_slug)}">${escape(b.fighter_b_name)}</a> ${hasPick?`<strong>${pct(b.fighter_b_probability)}${bPicked?' model pick':''}</strong>`:''}</p><p class="muted">${hasPick?`Locked CageMetrix Predictor ${escape(b.model_version)} win probability.`:'Prediction pending.'}</p><a class="button secondary" href="/fights/${b.id}">Prediction, stats & model explanation →</a></article>`;
  }).join('');
  const html=`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(title)}</title><meta name="description" content="${escape(description)}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${canonical}"><link rel="icon" href="/logo.svg" type="image/svg+xml"><link rel="stylesheet" href="/styles.css"><link rel="stylesheet" href="/explorer.css"><meta property="og:type" content="article"><meta property="og:title" content="${escape(title)}"><meta property="og:description" content="${escape(description)}"><meta property="og:url" content="${canonical}"><meta property="og:image" content="${SITE}/og.png"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${escape(title)}"><meta name="twitter:description" content="${escape(description)}"><meta name="twitter:image" content="${SITE}/og.png"><script type="application/ld+json">${jsonLd(schema)}</script><script type="application/ld+json">${jsonLd(breadcrumbs)}</script><style>.event-seo{max-width:1120px;margin:0 auto;padding:48px 24px}.event-seo .lede{max-width:780px}.event-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:18px;margin-top:28px}.event-fight{border:1px solid var(--line,#d8d8d8);border-radius:18px;padding:22px;background:var(--panel,#fff)}.event-fight h2{font-size:1.25rem;margin:.3rem 0 .7rem}.event-fight p{line-height:1.55}.event-nav{display:flex;gap:12px;flex-wrap:wrap;margin-top:24px}</style></head><body><header class="topbar"><a class="brand" href="/"><img class="brand-mark" src="/logo.svg" width="44" height="44" alt=""><span>CageMetrix</span></a><nav><a href="/predictions.html">Predictions</a><a href="/#rankings">Rankings</a><a href="/validation.html">Track record</a></nav></header><main class="event-seo"><p class="eyebrow">${escape(String(event.promotion||'UFC').toUpperCase())} · ${escape(prettyDate(event.event_date))}</p><h1>${escape(label)} Predictions: ${escape(matchup)}</h1><p class="lede">CageMetrix model picks and win probabilities for ${escape(label)}, headlined by ${escape(matchup)}. Every prediction is locked before the event and remains on the public record.</p><div class="event-nav"><a class="button primary" href="/predictions.html">All UFC predictions</a>${event.source_url?`<a class="button secondary" href="${escape(event.source_url)}" rel="nofollow noopener">Official UFC event page</a>`:''}</div><p class="model-note">Win probabilities are model estimates, not betting odds. Technical statistics remain UFC-specific; verified pre-UFC résumé context may inform low-sample fighters.</p><section><div class="section-heading"><div><p class="eyebrow">FULL CARD</p><h2>${escape(label)} fight predictions</h2></div><p>${bouts.length} matchups with preserved model probabilities and fight-specific breakdowns.</p></div><div class="event-grid">${cards||'<p>Fight predictions are being prepared.</p>'}</div></section></main><footer><span>CageMetrix</span><span>Transparent, opponent-adjusted MMA analytics.</span></footer></body></html>`;
...
export async function enhanceFighterPage(response:Response,env:Env,slug:string){
  ...
  const score=Number.isFinite(Number(rating?.cmr))?Number(rating.cmr).toFixed(1):null;
  ...
  const breadcrumbs={'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'CageMetrix',item:`${SITE}/`},{'@type':'ListItem',position:2,name:'UFC Rankings',item:`${SITE}/#rankings`},{'@type':'ListItem',position:3,name:fighter.name,item:canonical}]};
  return new HTMLRewriter()
    ...
    .on('#fighter-division',{element(el){el.setInnerContent(`${division} · CAGEMETRIX FIGHTER PROFILE`);}})
    ...
    .on('#fighter-rank',{element(el){el.setInnerContent(score?'Current opponent-adjusted CMR':'Rating pending');}})
    .on('head',{element(el){el.append(`<meta name="robots" content="index,follow,max-image-preview:large"><meta property="og:image" content="${SITE}/og.png"><meta name="twitter:image" content="${SITE}/og.png"><script type="application/ld+json">${jsonLd(schema)}</script><script type="application/ld+json">${jsonLd(breadcrumbs)}</script>`,{html:true});}})
    .transform(response);
}

export async function enhanceFightPage(response:Response,env:Env,id:string){
  if(!response.ok||!response.headers.get('content-type')?.includes('text/html'))return response;
  const payload=await getFight(id,env);
  ...
  const a=names?.a?.name||bout.fighter_a_name,b=names?.b?.name||bout.fighter_b_name;
  const eventLabel=event?eventSeoName(event):String(bout.event_name||'UFC');
  ...
  const canonical=`${SITE}/fights/${id}`;
  const eventCanonical=event?.slug?`${SITE}/events/${event.slug}`:undefined;
  const schema:any={'@context':'https://schema.org','@type':'SportsEvent',name:`${a} vs ${b}`,url:canonical,startDate:event?.starts_at||bout.starts_at||bout.event_date,eventStatus:eventSchemaStatus(bout.status),sport:'Mixed Martial Arts',organizer:{'@type':'Organization',name:'UFC',url:'https://www.ufc.com/'},competitor:[{'@type':'Person',name:a,url:`${SITE}/fighters/${bout.fighter_a_slug}`},{'@type':'Person',name:b,url:`${SITE}/fighters/${bout.fighter_b_slug}`}],...(event&&locationSchema(event)?{location:locationSchema(event)}:{}),...(eventCanonical?{superEvent:{'@type':'SportsEvent',name:eventLabel,url:eventCanonical}}:{})};
  const breadcrumbs={'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'CageMetrix',item:`${SITE}/`},{'@type':'ListItem',position:2,name:eventLabel,item:eventCanonical||`${SITE}/predictions.html`},{'@type':'ListItem',position:3,name:`${a} vs ${b}`,item:canonical}]};
  const eventHtml=eventCanonical?`<a href="${eventCanonical}">${escape(eventLabel)}</a>${event?.event_date?` · ${escape(prettyDate(event.event_date))}`:''}`:`${escape(eventLabel)}${event?.event_date?` · ${escape(prettyDate(event.event_date))}`:''}`;
  const probabilityHtml=`<p class="lede"><strong>${escape(a)} ${pct(prediction.fighter_a_probability)}</strong> · <strong>${escape(b)} ${pct(prediction.fighter_b_probability)}</strong></p><p class="muted">Locked Predictor ${escape(prediction.model_version)} win probabilities. Model estimates, not betting odds.</p>`;
  return new HTMLRewriter()
    .on('title',{element(el){el.setInnerContent(title);}})
    .on('meta[name="description"]',{element(el){el.setAttribute('content',description);}})
    .on('meta[property="og:title"]',{element(el){el.setAttribute('content',title);}})
    ...
    .on('#fight-context',{element(el){el.setInnerContent(`${escape(a)} vs ${escape(b)} matchup analysis, opponent-adjusted ratings and preserved pre-fight model inputs.`,{html:true});}})
