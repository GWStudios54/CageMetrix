...
export async function enhanceFighterPage(response:Response,env:Env,slug:string){
  ...
  return new HTMLRewriter()
    ...
    .on('head',{element(el){el.append(`<meta name="robots" content="index,follow,max-image-preview:large"><meta property="og:image" content="${SITE}/og.png"><meta name="twitter:image" content="${SITE}/og.png"><script type="application/ld+json">${jsonLd(schema)}</script><script type="application/ld+json">${jsonLd(crumbs)}</script>`,{html:true});}})
    .transform(response);
}
...
export async function enhanceFightPage(response:Response,env:Env,id:string){
  ...
  const transformed=new HTMLRewriter()
    ...
    .on('head',{element(el){el.append(`<meta name="robots" content="index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1"><meta property="og:site_name" content="CageMetrix"><meta property="og:locale" content="en_US"><meta property="og:image" content="${SITE}/og.png"><meta name="twitter:image" content="${SITE}/og.png"><script type="application/ld+json">${jsonLd(structured)}</script>`,{html:true});}})
    .transform(response);
