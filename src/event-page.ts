...
function scoutUrl(name:unknown){return `${SITE}/scout?q=${encodeURIComponent(`Scout ${String(name||'').trim()}`)}`;}
function schemaPerson(name:unknown){return {'@type':'Person',name:String(name||''),url:scoutUrl(name)};}
...
export async function eventPage(_request:Request,env:Env,slug:string){
  ...
  const rows=await env.DB.prepare(`SELECT b.id,b.bout_order,b.weight_class,b.status,b.winner_id,b.result_method,b.result_round,b.result_time_seconds,
    ...
    WHERE b.event_id=? ORDER BY b.bout_order,b.id`).bind(event.id).all<Row>();
  ...
  const title=`${event.name} Fight Card, Fighters & Scout Reports | ${BRAND_NAME}`;
  const description=`Scout ${event.name}${event.event_date?` on ${prettyDate(event.event_date)}`:''}${locationText?` at ${locationText}`:''}. Research the promotion, fighters, records and scouting context on ${BRAND_NAME}.`.slice(0,190);
  ...
  const main=bouts[0];
  const mainPeople=main?[schemaPerson(main.fighter_a_name),schemaPerson(main.fighter_b_name)]:undefined;
  const schema=location?{
    ...
    ...(bouts.length?{subEvent:bouts.map((b:Row)=>{const people=[schemaPerson(b.fighter_a_name),schemaPerson(b.fighter_b_name)];return {'@type':'SportsEvent',name:`${b.fighter_a_name} vs ${b.fighter_b_name}`,url:`${canonical}#bout-${b.id}`,startDate:event.starts_at||event.event_date,eventStatus:schemaStatus(b.status),sport:'Mixed Martial Arts',description:`${b.fighter_a_name} vs ${b.fighter_b_name} at ${event.name}. Fighter and scouting context from ${BRAND_NAME}.`,organizer,location,performer:people,competitor:people};})}: {})
  }:null;
  ...
  const cards=bouts.map((b:Row,index)=>`<article class="scout-bout" id="bout-${Number(b.id)}"><div class="scout-bout-meta"><span>${esc(b.weight_class||'MMA bout')}</span><span>${index===0?'Featured bout':`Bout ${index+1}`}</span></div><div class="scout-bout-matchup"><a href="/scout?q=${encodeURIComponent(`Scout ${String(b.fighter_a_name)}`)}"><strong>${esc(b.fighter_a_name)}</strong><small>Scout fighter →</small></a><span>VS</span><a href="/scout?q=${encodeURIComponent(`Scout ${String(b.fighter_b_name)}`)}"><strong>${esc(b.fighter_b_name)}</strong><small>Scout fighter →</small></a></div><div class="scout-bout-footer"><span>${esc(boutResult(b))}</span><a href="/scout?q=${encodeURIComponent(`Compare ${String(b.fighter_a_name)} and ${String(b.fighter_b_name)} as MMA prospects and fighters`)}">Compare in Scout AI →</a></div></article>`).join('');
