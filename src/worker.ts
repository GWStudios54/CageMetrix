...
import {adminDashboardPage,adminModerateForumPost,adminModeratePost,adminOverviewV2,adminResolveForumReports,adminResolveReports} from './admin.ts';
...
function methodNotAllowed(){return new Response(JSON.stringify({error:'method_not_allowed'}),{status:405,headers:JSON_HEADERS});}
...
async function forecastsWithFans(_request:Request,env:Env){
  ...
  payload.data=(payload.data||[]).map((row:any)=>{const f:any=fans.get(Number(row.bout_id));const total=Number(f?.total||0),a=Number(f?.a_votes||0),b=Number(f?.b_votes||0);return {...row,fan_total:total,fan_a_pct:total?a/total:null,fan_b_pct:total?b/total:null,fan_consensus_pick:a===b?null:a>b?'a':'b'};});
...
export default {
  ...
  async fetch(request:Request,env:Env,context:ExecutionContext):Promise<Response>{
    ...
    if(request.method==='GET'&&(url.pathname==='/admin'||url.pathname==='/admin/')){if(url.pathname.endsWith('/'))return Response.redirect(new URL('/admin',request.url),308);return renderPage(adminDashboardPage(request,env),request,env);}
    if(request.method==='GET'&&(url.pathname==='/watchlist'||url.pathname==='/watchlist/')){if(url.pathname.endsWith('/'))return Response.redirect(new URL('/watchlist',request.url),308);return renderPage(watchlistPage(request,env),request,env);}
    if(request.method==='GET'&&(url.pathname==='/community'||url.pathname==='/community/')){if(url.pathname.endsWith('/'))return Response.redirect(new URL('/community',request.url),308);return renderPage(communityHomePage(request,env),request,env);}
    if(request.method==='GET'&&(url.pathname==='/forum'||url.pathname==='/forum/')){if(url.pathname.endsWith('/'))return Response.redirect(new URL('/forum',request.url),308);return renderPage(normalizeForumSeo(await forumHomePage(request,env),'/forum',true),request,env);}

    const forumEventMatch=url.pathname.match(/^\/forum\/event\/([a-z0-9-]{1,180})\/?$/);
    if(request.method==='GET'&&forumEventMatch){if(url.pathname.endsWith('/'))return Response.redirect(new URL(`/forum/event/${forumEventMatch[1]}`,request.url),308);return renderPage(normalizeForumSeo(await forumScopedThreadPage(request,env,'event',forumEventMatch[1]),`/forum/event/${forumEventMatch[1]}`,false),request,env);}
    const forumFightMatch=url.pathname.match(/^\/forum\/fight\/([1-9]\d*)\/?$/);
    if(request.method==='GET'&&forumFightMatch){if(url.pathname.endsWith('/'))return Response.redirect(new URL(`/forum/fight/${forumFightMatch[1]}`,request.url),308);return renderPage(normalizeForumSeo(await forumScopedThreadPage(request,env,'fight',forumFightMatch[1]),`/forum/fight/${forumFightMatch[1]}`,false),request,env);}
    const forumThreadMatch=url.pathname.match(/^\/forum\/thread\/([1-9]\d*)\/?$/);
    if(request.method==='GET'&&forumThreadMatch){if(url.pathname.endsWith('/'))return Response.redirect(new URL(`/forum/thread/${forumThreadMatch[1]}`,request.url),308);return renderPage(normalizeForumSeo(await forumThreadPage(request,env,forumThreadMatch[1]),`/forum/thread/${forumThreadMatch[1]}`,false),request,env);}
    ...
    const forumApiThread=url.pathname.match(/^\/api\/forum\/threads\/([1-9]\d*)(?:\/posts)?$/);
    if(forumApiThread){if(request.method==='GET'&&!url.pathname.endsWith('/posts'))return forumThread(request,env,forumApiThread[1]);if(request.method==='POST'&&url.pathname.endsWith('/posts'))return createForumPost(request,env,forumApiThread[1]);return methodNotAllowed();}
    const forumPostAction=url.pathname.match(/^\/api\/forum\/posts\/([1-9]\d*)\/(react|report)$/);
    if(forumPostAction){if(forumPostAction[2]==='react'&&request.method==='PUT')return reactForumPost(request,env,forumPostAction[1]);if(forumPostAction[2]==='report'&&request.method==='POST')return reportForumPost(request,env,forumPostAction[1]);return methodNotAllowed();}
    ...
    if(url.pathname==='/api/community/me'){if(request.method==='GET')return getCommunityMe(request,env);if(request.method==='PATCH')return updateCommunityMe(request,env);return methodNotAllowed();}
    if(url.pathname==='/api/community/watchlist'&&request.method==='GET')return getWatchlist(request,env);
    const followMatch=url.pathname.match(/^\/api\/community\/fighters\/([a-z0-9-]{1,180})\/follow$/);
    if(followMatch){if(request.method==='GET')return fighterFollowStatus(request,env,followMatch[1]);if(request.method==='PUT')return setFighterFollow(request,env,followMatch[1]);return methodNotAllowed();}
    ...
    if(communityEventMatch){if(!communityEventMatch[2]&&request.method==='GET')return communityEvent(request,env,communityEventMatch[1]);if(communityEventMatch[2]&&request.method==='PUT')return saveCommunityPick(request,env,communityEventMatch[1],communityEventMatch[2]);return methodNotAllowed();}
    if(url.pathname==='/api/community/discussion'){if(request.method==='GET')return discussion(request,env);if(request.method==='POST')return createDiscussionPost(request,env);return methodNotAllowed();}
    const reactMatch=url.pathname.match(/^\/api\/community\/posts\/([1-9]\d*)\/(react|report)$/);
    if(reactMatch){if(reactMatch[2]==='react'&&request.method==='PUT')return reactCommunityPost(request,env,reactMatch[1]);if(reactMatch[2]==='report'&&request.method==='POST')return reportCommunityPost(request,env,reactMatch[1]);return methodNotAllowed();}
    ...
    const notesMatch=url.pathname.match(/^\/api\/fights\/([1-9]\d*)\/notes$/);
    if(notesMatch){if(request.method==='GET')return getContributorNotes(request,env,notesMatch[1]);if(request.method==='PUT')return saveContributorNote(request,env,notesMatch[1]);return methodNotAllowed();}
    const fanMatch=url.pathname.match(/^\/api\/fights\/([1-9]\d*)\/(fans|fan-prediction|fan-scorecard)$/);
    if(fanMatch){if(fanMatch[2]==='fans'&&request.method==='GET')return getFanSummary(request,env,fanMatch[1]);if(fanMatch[2]==='fan-prediction'&&request.method==='PUT')return saveFanPrediction(request,env,fanMatch[1]);if(fanMatch[2]==='fan-scorecard'&&request.method==='PUT')return saveFanScorecard(request,env,fanMatch[1]);return methodNotAllowed();}
    if(url.pathname==='/api/contributors/me'){if(request.method==='GET'){const who=await contributor(request,env.DB);return who?new Response(JSON.stringify(who),{headers:JSON_HEADERS}):new Response(JSON.stringify({error:'Invalid or revoked publishing key.'}),{status:401,headers:JSON_HEADERS});}if(request.method==='PATCH')return updateContributorProfile(request,env.DB);return methodNotAllowed();}
    if(url.pathname==='/api/contributors/device'){if(request.method==='POST')return rememberContributor(request,env.DB);if(request.method==='DELETE')return forgetContributor();return methodNotAllowed();}
    ...
    if(request.method==='GET'&&fighterPath){const slug=decodeURIComponent(fighterPath[1]);let response=await base.fetch(routed,env,context);response=await enhanceFighterPage(response,env,slug);response=await enhanceFighterFollow(response,env,slug);return renderPage(response,request,env);}
    const fightPath=url.pathname.match(/^\/fights\/([1-9]\d*)\/?$/);
    if(request.method==='GET'&&fightPath){let response=await base.fetch(routed,env,context);response=await enhanceFightPage(response,env,fightPath[1]);response=await enhanceFightCommunity(response,env,fightPath[1]);response=separateDiscussion(response,'fight',fightPath[1]);return renderPage(response,request,env);}
    return renderPage(base.fetch(routed,env,context),request,env);
  }
