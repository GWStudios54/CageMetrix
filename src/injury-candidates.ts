...
export async function injuryCandidatesAdminApi(request:Request,env:Env){
  ...
  if(request.method==='GET'){
    ...
    return json({data:rows.results||[],meta:{status,limit,count:rows.results?.length||0,policy:'Discovery candidates are private review leads, not published injury/availability facts about any fighter.'}});
  }
  ...
  const candidate=await env.DB.prepare(`SELECT id,review_status FROM injury_intel_candidates WHERE id=? LIMIT 1`).bind(id).first<Row>();if(!candidate)return json({error:'candidate_not_found'},404);
  await env.DB.prepare(`UPDATE injury_intel_candidates SET review_status=?,reviewed_at=CURRENT_TIMESTAMP,notes=? WHERE id=?`).bind(status,String(value.notes||'').trim().slice(0,2000)||null,id).run();
