...
export async function publicSitemap(env:Env){
  const [fighters,events,promotions,regionalFighters]=await Promise.all([
  ...
  for(const row of regionalFighters.results||[])urls.push([`${SITE}/scout/fighters/${encodeURIComponent(String(row.profile_slug))}`,dateOnly(row.last_fight_date),'weekly','0.82']);
