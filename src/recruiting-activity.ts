...
const KIND=new Set(['all','availability','contract','representation']);
...
async function activityRows(env:Env,kind:string,limit=75){
  const [contracts,representation]=await Promise.all([
    kind==='representation'?Promise.resolve({results:[]}):env.DB.prepare(`
    ...
    `).bind(limit).all<Row>(),
    kind==='availability'||kind==='contract'?Promise.resolve({results:[]}):env.DB.prepare(`
...
export async function recruitingActivityPage(request:Request,env:Env){
  ...
  const bodyHtml=`<section class="recruit-hero opening-hero"><a class="back-link" href="/recruiting">← Recruiting board</a><span class="eyebrow">MARKET ACTIVITY</span><h1>Who just became available.</h1><p>A chronological feed of publicly reported contract and representation changes across every fighter MMA Scouts tracks -- new free agents, releases, expirations, signings and agency changes -- so you don't have to already know a name to spot an opportunity.</p></section>
  <section class="intel-filter-panel"><form method="get" action="/recruiting/activity"><label>Show<select name="kind">${['all','availability','contract','representation'].map(v=>`<option value="${v}"${kind===v?' selected':''}>${v==='all'?'Everything':v==='availability'?'Free agency, release & expiration':v==='contract'?'All contract events':'Representation changes'}</option>`).join('')}</select></label><button class="button primary" type="submit">Filter feed</button></form></section>
