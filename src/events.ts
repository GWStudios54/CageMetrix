...
function eventCard(row:Row){
  ...
  return `<a class="event-directory-card" href="${href}"><div class="event-directory-meta"><span>${esc(promo)}</span><time datetime="${esc(row.event_date)}">${esc(prettyDate(row.event_date))}</time></div><h3>${esc(row.name)}</h3><p>${esc(location(row)||'Venue to be announced')}</p><span class="event-directory-arrow">Event details →</span></a>`;
}
...
</style></head><body><header class="topbar"><a class="brand" href="/" aria-label="${esc(BRAND_NAME)} home"><img class="brand-mark" src="/logo.svg" alt="" width="44" height="44"><span>${esc(BRAND_NAME)}</span></a></header><main class="event-directory"><section class="event-directory-hero"><span class="eyebrow">GLOBAL MMA CALENDAR</span><h1>What’s next in MMA.</h1><p>Official event tracking across major and feeder promotions. Open an event for its date, venue, fight-card research and promotion context; UFC cards also include locked MMA Scouts win probabilities when available.</p><div class="directory-hero-actions"><a class="button primary" href="/promotions">Browse promotions</a><a class="button secondary" href="/predictions.html">UFC predictions</a></div></section>${groupHtml||'<p class="event-directory-empty">The next official cards are being synchronized now.</p>'}${recentHtml?`<section class="event-directory-section"><div class="event-directory-heading"><span class="eyebrow">RECENT</span><h2>Recently tracked events</h2></div><div class="event-directory-grid">${recentHtml}</div></section>`:''}</main><footer><span>${esc(BRAND_NAME)}</span><span>The MMA research engine.</span></footer></body></html>`;
