...
function score(value:unknown){return Number.isFinite(Number(value))?Number(value).toFixed(1):'—';}
function percent(value:unknown){return Number.isFinite(Number(value))?`${Math.round(Number(value))}%`:'—';}
...
async function promotionSummary(env:Env){return env.DB.prepare(`
  ...
  LEFT JOIN scout_active_global_profiles p ON p.current_promotion_slug=sp.slug
...
export async function promotionPage(request:Request,env:Env,slug:string){
  const response=await promotionApi(new Request(request.url,{method:'GET'}),env,slug);if(!response.ok)return new Response('Not found',{status:404});const payload:any=await response.json(),promo:Row=payload.promotion,rows:Row[]=payload.data||[];
  ...
  const body=`<section class="directory-hero directory-promo-hero"><a class="directory-back" href="/promotions">← All promotions</a><span class="eyebrow">${escape(promo.region)} · ${escape(String(promo.scope).replaceAll('_',' '))}</span><h1>${escape(promo.name)}</h1><p>${escape(promo.country||'')} · ${rows.length} recently active indexed fighters${promo.official_url?` · <a href="${escape(promo.official_url)}" rel="nofollow noopener">Official site ↗</a>`:''}</p>${divisions.length?`<div class="directory-chips">${divisions.map(d=>`<span>${escape(d)}</span>`).join('')}</div>`:''}</section><section class="directory-roster"><div class="directory-section-head"><span class="eyebrow">SCOUT ROSTER</span><h2>Fighters by Global Scout Rating</h2><p>Opponent strength travels with the fighter through the global fight graph. The promotion name itself adds no points.</p></div><div class="directory-fighter-list">${roster||'<p class="directory-empty">No recent indexed roster yet.</p>'}</div></section>`;
