...
export async function recruitingPage(request:Request,env:Env){
  ...
  const bodyHtml=`<section class="recruit-hero"><span class="eyebrow">RECRUITING WORKSPACE</span><h1>Openings, shortlists, and the intel behind the call.</h1><p>Create a recruiting need, generate candidates from the verified fighter graph, then track review, outreach and intelligence gaps privately.</p><div class="hero-actions"><a class="button primary" href="/recruiting/intel">Open intelligence queue →</a><a class="button secondary" href="/recruiting/activity">Market activity →</a><a class="button secondary" href="/recruiting/watchlist">Global watchlist →</a><a class="button secondary" href="/recruiting/contracts">Review contract leads →</a><a class="button secondary" href="/recruiting/camps">Review camp leads →</a><a class="button secondary" href="/recruiting/coaches">Review coach leads →</a><a class="button secondary" href="/recruiting/antidoping">Review anti-doping leads →</a><a class="button secondary" href="/recruiting/injuries">Review injury leads →</a><a class="button secondary" href="/recruiting/amateur-records">Review amateur-record leads →</a></div></section>
...
export async function recruitingIntelQueuePage(request:Request,env:Env){
  ...
  const rows=(await env.DB.prepare(`
    SELECT c.*,r.scout_rating global_rating,r.evidence_strength,
           p.career_wins,p.career_bouts,p.ko_tko_wins,p.submission_wins,p.title_fight_bouts,p.title_fight_wins,p.last_five_wins,p.last_five_losses,p.finish_round_sum,p.first_round_finishes,p.times_finished,
