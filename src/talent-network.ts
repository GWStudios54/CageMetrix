...
function talentWhere(url:URL){
  ...
  const activeMonths=number(url.searchParams.get('active_months'),1,60);
  const minFinishRate=number(url.searchParams.get('min_finish_rate'),0,100);
  const titleExperience=(url.searchParams.get('title_experience')||'').trim()==='yes';
  ...
  if(activeMonths!==null){clauses.push(`p.last_fight_date IS NOT NULL AND date(substr(p.last_fight_date,1,10))>=date('now',?)`);binds.push(`-${Math.round(activeMonths)} months`);}
  if(minFinishRate!==null){clauses.push(`p.career_wins>0 AND (CAST(p.ko_tko_wins+p.submission_wins AS REAL)/p.career_wins)*100>=?`);binds.push(minFinishRate);}
  if(titleExperience)clauses.push('p.title_fight_bouts>0');
  return {clauses,binds,filters:{q:q||null,division:division||null,promotion:promotion||null,region:region||null,country:country||null,management:management||null,contract:contract||null,opportunity:opportunity||null,age_max:ageMax,age_min:ageMin,min_rating:minRating,min_evidence:minEvidence,min_wins:minWins,active_months:activeMonths,min_finish_rate:minFinishRate,title_experience:titleExperience?true:null}};
}
...
export async function talentPage(request:Request,env:Env){
  ...
  const isAdmin=!!await adminAccount(request,env.DB);
  ...
  const watchButton=(row:Row)=>isAdmin?`<button class="button secondary" type="button" data-watch="${esc(row.profile_slug)}">☆ Watch</button>`:'';
  const cards=rows.map(row=>`<article class="talent-card"><div class="talent-card-main"><div><span class="eyebrow">${esc(row.current_weight_class||'Unknown division')}${row.promotion_name?` · ${esc(row.promotion_name)}`:''}</span><h2><a href="/scout/fighters/${esc(row.profile_slug)}">${esc(row.fighter_name)}</a></h2><p>${esc(record(row))}${age(row.dob)!==null?` · Age ${age(row.dob)}`:''}${row.gym?` · ${esc(row.gym)}`:''}</p></div><div class="talent-rating"><small>GLOBAL RATING</small><strong>${score(row.global_rating)}</strong><span>Evidence ${percent(row.evidence_strength)}</span></div></div><div class="talent-statuses">${statusLabel(row)}${contractLabel(row.contract_status)}${opportunityTags(row)}${fightProfileTags(row)}</div>${row.agency_slug?`<p class="talent-management-link">Management: <a href="/management/${esc(row.agency_slug)}">${esc(row.agency_name)}</a></p>`:''}${row.base_country||row.base_city?`<p class="talent-location">Base: ${esc([row.base_city,row.base_region,row.base_country].filter(Boolean).join(', '))}</p>`:''}${row.last_fight_date?`<p class="talent-location">Last recorded fight: ${esc(pretty(row.last_fight_date))}</p>`:''}<div class="talent-card-actions"><a class="button secondary" href="/scout/fighters/${esc(row.profile_slug)}">Open recruiting file →</a>${watchButton(row)}</div></article>`).join('');
  const body=`<section class="talent-hero"><span class="eyebrow">RECRUITING INTELLIGENCE</span><h1>Find the right fighter. Know who to call.</h1><p>Build an evidence-backed shortlist by division, age, performance, recent activity, geography, representation and explicitly verified opportunity status. MMA Scouts separates what is known from what still needs verification.</p><div class="talent-hero-links"><a class="button secondary" href="/management">Management directory</a><a class="button secondary" href="/promotions">Promotion network</a></div></section>
  <section class="talent-search-panel"><form class="talent-form" method="get" action="/talent"><label>Name or gym<input name="q" value="${esc(get('q'))}" placeholder="Fighter or gym"></label><label>Division<input name="weight_class" value="${esc(get('weight_class'))}" placeholder="Lightweight"></label><label>Promotion<select name="promotion"><option value="">Any promotion</option>${promotions.map(p=>`<option value="${esc(p.slug)}"${selected('promotion',p.slug)}>${esc(p.name)}</option>`).join('')}</select></label><label>Region<select name="region"><option value="">Any region</option>${['United States','Europe','Asia'].map(v=>`<option${selected('region',v)}>${esc(v)}</option>`).join('')}</select></label><label>Country<input name="country" value="${esc(get('country'))}" placeholder="United States"></label><label>Min age<input name="age_min" type="number" min="14" max="60" value="${esc(get('age_min'))}" placeholder="18"></label><label>Max age<input name="age_max" type="number" min="14" max="60" value="${esc(get('age_max'))}" placeholder="25"></label><label>Min pro wins<input name="min_wins" type="number" min="0" max="100" value="${esc(get('min_wins'))}" placeholder="5"></label><label>Min Global Rating<input name="min_rating" type="number" min="0" max="100" step="1" value="${esc(get('min_rating'))}" placeholder="70"></label><label>Min evidence<input name="min_evidence" type="number" min="0" max="100" step="1" value="${esc(get('min_evidence'))}" placeholder="50"></label><label>Recent activity<select name="active_months"><option value="">Any activity</option><option value="6"${selected('active_months','6')}>Fought in last 6 months</option><option value="12"${selected('active_months','12')}>Fought in last 12 months</option><option value="18"${selected('active_months','18')}>Fought in last 18 months</option><option value="24"${selected('active_months','24')}>Fought in last 24 months</option></select></label><label>Management<select name="management"><option value="">Any status</option><option value="unmanaged"${selected('management','unmanaged')}>Verified unmanaged</option><option value="represented"${selected('management','represented')}>Represented</option><option value="unknown"${selected('management','unknown')}>Unknown</option></select></label><label>Contract<select name="contract"><option value="">Any status</option><option value="free_agent"${selected('contract','free_agent')}>Verified free agent</option><option value="non_exclusive"${selected('contract','non_exclusive')}>Non-exclusive</option><option value="under_contract"${selected('contract','under_contract')}>Under contract</option><option value="unknown"${selected('contract','unknown')}>Unknown</option></select></label><label>Opportunity<select name="opportunity"><option value="">Any</option><option value="fights"${selected('opportunity','fights')}>Open to fights</option><option value="management"${selected('opportunity','management')}>Open to management</option><option value="team"${selected('opportunity','team')}>Open to teams</option></select></label><label>Min finish rate<input name="min_finish_rate" type="number" min="0" max="100" step="1" value="${esc(get('min_finish_rate'))}" placeholder="50"></label><label>Title fights<select name="title_experience"><option value="">Any</option><option value="yes"${selected('title_experience','yes')}>Has fought for a title</option></select></label><button class="button primary" type="submit">Find candidates</button></form><p class="talent-policy"><strong>Verification rule:</strong> absence of a management or contract record never means unmanaged or free agent. Those labels only appear when a public source or verified profile explicitly supports them.</p></section>
  <section class="talent-results"><div class="talent-section-head"><span class="eyebrow">SEARCH RESULTS</span><h2>${rows.length} candidates shown</h2></div>${cards||'<div class="talent-empty">No candidates match those evidence-backed filters yet. Try widening the brief or removing an opportunity-status filter.</div>'}</section>`;
  const script=isAdmin?`<script>(()=>{document.querySelectorAll('[data-watch]').forEach(btn=>btn.addEventListener('click',async()=>{btn.disabled=true;try{const r=await fetch('/api/admin/recruiting/watchlist',{method:'POST',credentials:'same-origin',headers:{'content-type':'application/json'},body:JSON.stringify({profile_slug:btn.dataset.watch})}),j=await r.json();if(!r.ok)throw new Error(j.error||'Request failed');btn.textContent='✓ Watching';}catch(err){btn.textContent='Retry';btn.disabled=false;}}));})();</script>`:'';
  return new Response(shell('MMA Talent Search for Promotions, Managers & Teams | MMA Scouts','Search MMA fighters by Global Rating, age, division, promotion, representation and verified availability on MMA Scouts.','/talent',body,script),{headers:{'content-type':'text/html; charset=utf-8','cache-control':isAdmin?'private, no-store':'public, max-age=30, s-maxage=120'}});
}
