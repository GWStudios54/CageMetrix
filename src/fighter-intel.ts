...
export async function enhanceFighterIntel(response:Response,env:Env,slug:string){
  ...
  const contact=c.professional_contact_url?`<p><a class="button secondary" href="${esc(c.professional_contact_url)}" rel="nofollow noopener">${c.professional_contact_kind==='management_agency'?'Management contact path':'Public professional contact'} ↗</a></p>`:'';
