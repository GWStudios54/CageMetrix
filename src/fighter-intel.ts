...
const CORE_FIELDS:[string,string,(row:Row)=>boolean][]=[
  ...
  ['Fight availability','open_to_fights',r=>r.open_to_fights!=='unknown'],['Management availability','open_to_management',r=>r.open_to_management!=='unknown'],['Team availability','open_to_team',r=>r.open_to_team!=='unknown'],['Professional base','base',r=>Boolean(r.base_city||r.base_region||r.base_country)],['Professional contact','public_contact_url',r=>Boolean(r.public_contact_url)]
];
...
export async function fighterIntelApi(request:Request,env:Env,slug:string){
  ...
  const [facts,events,management,sources]=await Promise.all([
    ...
    env.DB.prepare(`SELECT agency_slug,agency_name,agency_website,manager_name,started_at,source_url,source_type,confidence,verified_at,last_checked_at FROM scout_current_management WHERE source_key=? AND source_fighter_id=? LIMIT 1`).bind(coverage.source_key,coverage.source_fighter_id).first<Row>(),
  ...
  return json({
    ...
    policy:{unknown_remains_unknown:true,professional_public_sources_only:true,no_private_contact_collection:true,no_rating_bonus_from_management:true}
  });
}
