...
test('fighter watchlists are persistent, personalized, and wired into fighter pages',()=>{
  ...
  assert.match(worker,/fighterFollowStatus/);
  assert.match(worker,/setFighterFollow/);
...
test('admin panel covers both fight-night and standalone forum reports',()=>{
  ...
  assert.match(worker,/adminModerateForumPost/);
  assert.match(worker,/adminResolveForumReports/);
