...
import {candidateRows,detectContractSignal,exactFighterMatches,hasContractSignal,normalizeContractText,parseContractListing} from '../scripts/lib/contract-intel-discovery.mjs';
...
test('candidate discovery queues evidence without publishing contract events',()=>{
  ...
  const rows=candidateRows({title:'Jane Doe signs new contract',url:'https://example.com/news/jane',summary:'Jane Doe has signed a new multi-fight contract.'},source,'Jane Doe will remain with the promotion.',profiles);
