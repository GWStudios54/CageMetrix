...
export function seed(db){
  db.prepare("INSERT INTO model_versions(id,name,version,parameters_json) VALUES(100,'CageMetrix Win Probability','0.1.0',?)").run(JSON.stringify(FORECAST_PARAMETERS));
  ...
  const snapshot=predictionSnapshot({a:fighterA,b:fighterB,ratingA,ratingB,probabilities,snapshotKey:'0.3.0:fixture',sourceMaxDate:'2026-08-29',lockedAt:locked});
  ...
  db.prepare("INSERT INTO predictions(id,bout_id,model_version_id,locked_at,fighter_a_probability,fighter_b_probability,picked_fighter_id,input_snapshot_key,notes) VALUES(6,2,101,?,?,?,?,?,'Result-based Elo probability; sample strength is separate from win chance.')").run(locked,legacyA,1-legacyA,1,snapshot.input_snapshot_key);
  ...
  return {snapshot:JSON.parse(JSON.stringify(snapshot)),probabilities,locked};
}
