...
import { buildObservations, careerAggregates, buildRatings } from './lib/model_v021.mjs';

const DB = 'cagemetrix';
const STATE_KEY = 'rating-recalibration-0.2.1-2026-06-27';
...
function applyStatements(dir, statements, statementsPerFile = 25) {
  for (let i = 0; i < statements.length; i += statementsPerFile) {
    ...
    const path = join(dir, `v021-ratings-${String(part).padStart(3, '0')}.sql`);
    writeFileSync(path, `PRAGMA foreign_keys=ON;\n${statements.slice(i, i + statementsPerFile).join('\n')}\n`, 'utf8');
    console.log(`Applying CageMetrix v0.2.1 ratings part ${part}…`);
...
async function main() {
  try {
    ...
    if (status.includes('complete')) {
      console.log('CageMetrix v0.2.1 recalibration already complete.');
      return;
    }
  } catch {
  ...
  console.log('Recalculating CageMetrix v0.2.1 confidence and leaderboard scores…');
  ...
  const modelParameters = {
    ...
    leaderboard_reliability: '0.25 + 0.75 * ranking_confidence',
    provisional: 'fewer than 5 rated bouts or ranking confidence below 55%',
    low_bout_penalty: '3 CMR points per missing bout below 5',
    component_scores: 'retain v0.2 exposure shrinkage'
  };
  ...
  const modelSql = `INSERT OR IGNORE INTO model_versions (
    ...
    'v0.2.1 confidence calibration: preserves exposure-shrunk components while preventing dominant finishers from being treated like tiny-sample prospects.',
  ...
  const dir = mkdtempSync(join(tmpdir(), 'cagemetrix-v021-'));
  try {
  ...
  console.log(`CageMetrix v0.2.1 recalibration complete: ${ratingRows.length} ratings through ${sourceMaxDate}.`);
}
