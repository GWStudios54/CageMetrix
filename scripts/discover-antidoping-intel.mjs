...
import {ANTIDOPING_DISCOVERY_SOURCES,articleText,antidopingCandidateRows,parseAntidopingListing} from './lib/antidoping-intel-discovery.mjs';
...
const target=remote?'--remote':'--local',cache='.cache/antidoping-intel';mkdirSync(cache,{recursive:true});
...
async function fetchText(url,attempts=3){
  let last;for(let attempt=1;attempt<=attempts;attempt++)try{
    const response=await fetch(url,{redirect:'follow',signal:AbortSignal.timeout(35000),headers:{'accept':'text/html,application/xhtml+xml,application/rss+xml,application/xml;q=0.9,*/*;q=0.5','accept-language':'en-US,en;q=0.8','user-agent':'Mozilla/5.0 (compatible; MMA Scouts anti-doping research; +https://mmascouts.com/)'}});
...
for(const source of ANTIDOPING_DISCOVERY_SOURCES){
  try{
    const listing=await fetchText(source.url),listed=parseAntidopingListing(listing,source),articleSeen=new Set(),articles=listed.filter(article=>{if(!article?.url||articleSeen.has(article.url))return false;articleSeen.add(article.url);return true;}).slice(0,80),signalRows=[];
    ...
    for(const [index,article] of articles.entries()){
      try{
        ...
        const rows=dry?[]:antidopingCandidateRows(article,source,body,profiles);discovered.push(...rows);
    ...
    console.log(`${source.publisher}: ${articles.length} anti-doping-signal article(s)${dry?'':`, ${signalRows.reduce((sum,row)=>sum+Number(row.matched_candidates||0),0)} fighter candidate(s)`}`);
...
if(dry){if(!summary.sources_ok)throw new Error('All anti-doping discovery sources failed');console.log(`Anti-doping discovery probe found ${summary.signal_articles} signal article(s) across ${summary.sources_ok}/${ANTIDOPING_DISCOVERY_SOURCES.length} reachable sources.`);process.exit(0);}

const sql=[];
for(const row of unique){sql.push(`INSERT OR IGNORE INTO antidoping_intel_candidates(candidate_key,source_url,source_title,publisher,published_at,source_type,fighter_name,normalized_name,source_key,source_fighter_id,detected_event_type,detected_summary,extraction_method,review_status,discovered_at) VALUES(${q(row.candidateKey)},${q(row.sourceUrl)},${q(row.sourceTitle)},${q(row.publisher)},${q(row.publishedAt)},${q(row.sourceType)},${q(row.fighterName)},${q(row.normalizedName)},${q(row.sourceKey)},${q(row.sourceFighterId)},${q(row.detectedEventType)},${q(row.detectedSummary)},${q(row.extractionMethod||'signal_block_scoped_subject_v1')},${q(row.reviewStatus)},${q(checkedAt)});`);}
if(sql.length){writeFileSync(`${cache}/candidates.sql`,sql.join('\n')+'\n');wrangler(['d1','execute','cagemetrix',target,'--file',`${cache}/candidates.sql`]);}
const queue=query(`SELECT review_status,COUNT(*) count FROM antidoping_intel_candidates GROUP BY review_status ORDER BY review_status`);
summary.queue=queue;writeFileSync(`${cache}/summary.json`,JSON.stringify(summary,null,2)+'\n');
console.log(`Anti-doping discovery queued ${unique.length} signal-local fighter/article candidate(s); ${queue.map(row=>`${row.review_status}:${row.count}`).join(', ')} currently stored.`);
