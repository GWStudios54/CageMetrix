...
export function buildObservations(rows) {
  return rows.map((r, index) => {
    ...
    const weightClass = parseWeightClass(r.bout_type);
...
function divisionPriors(aggs) {
  const by = new Map();
  for (const a of aggs.values()) {
    const d = by.get(a.weightClass) || { minutes: 0, sigL: 0, sigAbs: 0, tdL: 0, tdAllowed: 0 };
    d.minutes += a.minutes;
    d.sigL += a.sigL;
    d.sigAbs += a.sigAbs;
    d.tdL += a.tdL;
    d.tdAllowed += a.tdAllowed;
    by.set(a.weightClass, d);
  }
  for (const [key, d] of by) {
    const m = Math.max(1, d.minutes);
    by.set(key, { sigProd: d.sigL / m, sigAllow: d.sigAbs / m, tdProd: d.tdL / m * 15, tdAllow: d.tdAllowed / m * 15 });
  }
  return by;
}
...
function computeElo(fightPairs) {
  ...
  for (const pair of ordered) {
    const a = pair.red.name, b = pair.blue.name;
    ...
    const k = 28 * (pair.red.finish ? 1.15 : 1);
    current.set(a, ra + k * (pair.red.won - expectedA));
    current.set(b, rb + k * ((1 - pair.red.won) - (1 - expectedA)));
  }
  return { current, opponentPre };
}

export function buildRatings(fightPairs, aggs) {
  const priors = divisionPriors(aggs);
  ...
  for (const pair of fightPairs) {
    for (const obs of [pair.red, pair.blue]) {
      const oppAgg = aggs.get(obs.opponent);
      ...
      p.sigOff.push([sigOff, w]); p.sigDef.push([sigDef, w]); p.tdOff.push([tdOff, w]); p.tdDef.push([tdDef, w]);
      p.grappling.push([grappling, w]); p.pace.push([pace, w]); p.finishing.push([finishing, w]); p.sos.push([opponentElo, w]);
      p.results.push({ date: obs.eventDate, won: obs.won, opponentElo });
      if (!p.lastFight || (obs.eventDate && obs.eventDate > p.lastFight)) { p.lastFight = obs.eventDate; p.weightClass = obs.weightClass; }
      perf.set(obs.name, p);
    }
  }
  ...
  const rows = [...perf.values()].map(p => ({
    ...p,
    sigOffRaw: weightedMean(p.sigOff), sigDefRaw: weightedMean(p.sigDef), tdOffRaw: weightedMean(p.tdOff), tdDefRaw: weightedMean(p.tdDef),
    grapplingRaw: weightedMean(p.grappling), paceRaw: weightedMean(p.pace), finishingRaw: weightedMean(p.finishing), sosRaw: weightedMean(p.sos), eloRaw: elo.get(p.name) ?? 1500
  }));
  ...
  for (const group of groups.values()) {
    const d = {
      sigOff: distribution(group.map(x => x.sigOffRaw)), sigDef: distribution(group.map(x => x.sigDefRaw)),
      tdOff: distribution(group.map(x => x.tdOffRaw)), tdDef: distribution(group.map(x => x.tdDefRaw)),
      grappling: distribution(group.map(x => x.grapplingRaw)), pace: distribution(group.map(x => x.paceRaw)),
      finishing: distribution(group.map(x => x.finishingRaw)), sos: distribution(group.map(x => x.sosRaw)), elo: distribution(group.map(x => x.eloRaw))
    };
    for (const row of group) {
      const strikingOffense = scoreFrom(row.sigOffRaw, d.sigOff);
      const strikingDefense = scoreFrom(row.sigDefRaw, d.sigDef);
      const wrestlingOffense = scoreFrom(row.tdOffRaw, d.tdOff);
      const wrestlingDefense = scoreFrom(row.tdDefRaw, d.tdDef);
      const grappling = scoreFrom(row.grapplingRaw, d.grappling);
      const pace = scoreFrom(row.paceRaw, d.pace);
      const finishing = scoreFrom(row.finishingRaw, d.finishing);
      const strengthOfSchedule = scoreFrom(row.sosRaw, d.sos);
      const resume = scoreFrom(row.eloRaw, d.elo);
      ...
      const recentForm = clamp(50 + recentRaw * 38, 5, 99);
      const confidence = clamp(100 * (1 - Math.exp(-row.minutes / 75)) * (0.55 + 0.45 * (1 - Math.exp(-row.bouts / 7))), 5, 99);
      const rawCmr = 0.56 * technical + 0.24 * resume + 0.10 * strengthOfSchedule + 0.10 * recentForm;
      const cmr = clamp(60 + (rawCmr - 60) * (0.35 + 0.65 * confidence / 100), 5, 99);
      ratings.push({
        ...row, strikingOffense, strikingDefense, wrestlingOffense, wrestlingDefense, grappling, pace, finishing,
        strengthOfSchedule, resume, technical, recentForm, confidence, cmr,
        components: {
