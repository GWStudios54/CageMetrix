...
const completedIdentityTotals=query(`SELECT COUNT(*) completed_sides,SUM(p.effective_source_fighter_id IS NOT NULL) resolved_sides,SUM(p.effective_source_fighter_id IS NULL) unresolved_sides FROM mma_effective_participants p JOIN mma_active_fights f ON f.source_key=p.source_key AND f.snapshot_id=p.snapshot_id AND f.source_fight_id=p.source_fight_id WHERE p.result IN ('W','L','D','NC') AND f.outcome<>'unknown' AND f.event_date<=date('now')`)[0]||{};
const completedSides=Number(completedIdentityTotals.completed_sides||0),resolvedSides=Number(completedIdentityTotals.resolved_sides||0),unresolvedSides=Number(completedIdentityTotals.unresolved_sides||0);
const identityCoveragePct=completedSides?Math.round((resolvedSides/completedSides)*10000)/100:100;
const unresolvedIdentityExamples=query(`SELECT p.fighter_name,COUNT(*) completed_sides FROM mma_effective_participants p JOIN mma_active_fights f ON f.source_key=p.source_key AND f.snapshot_id=p.snapshot_id AND f.source_fight_id=p.source_fight_id WHERE p.result IN ('W','L','D','NC') AND f.outcome<>'unknown' AND f.event_date<=date('now') AND p.effective_source_fighter_id IS NULL GROUP BY p.fighter_name ORDER BY completed_sides DESC,p.fighter_name LIMIT 100`);
...
const summary={checked_at:checkedAt,definition:'Complete means every safely attributable completed fighter-side bout reported by the active source snapshot is materialized. Unresolved source identities and source freshness are reported separately; absolute world completeness is not inferred.',totals,statuses,public_profile_coverage:publicCoverage,public_profiles_without_coverage_row:missingPublicCoverage,identity_resolution:{completed_sides:completedSides,resolved_sides:resolvedSides,unresolved_sides:unresolvedSides,resolved_pct:identityCoveragePct,unresolved_examples:unresolvedIdentityExamples},unsafe_self_identity_fights:unsafeSelfIdentity,remaining_safe_effective_sides_unmaterialized:remainingEffectiveMissing,profiles_needing_fresher_or_secondary_source:staleProfiles};
...
console.log(`History materialization audit passed for ${totals.fighters||0} master fighters at ${identityCoveragePct}% completed-side identity coverage. Freshness and unresolved identities remain independent requirements.`);
