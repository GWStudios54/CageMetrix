...
for(const agency of reachableAgencies){
  ...
  for(const item of agency.profileSources.filter(source=>source.status==='ok'))sql.push(`INSERT INTO management_agency_sources(agency_id,source_url,source_title,source_type,verified_at,last_checked_at) SELECT id,${q(item.url)},${q(`${agency.name} official profile`)},'official_profile',${q(checkedAt)},CURRENT_TIMESTAMP FROM management_agencies WHERE slug=${q(agency.slug)} ON CONFLICT(agency_id,source_url) DO UPDATE SET source_title=excluded.source_title,source_type=excluded.source_type,verified_at=excluded.verified_at,last_checked_at=CURRENT_TIMESTAMP;`);
  for(const item of agency.sources.filter(source=>source.status==='ok'))sql.push(`INSERT INTO management_agency_sources(agency_id,source_url,source_title,source_type,verified_at,last_checked_at) SELECT id,${q(item.url)},${q(`${agency.name} official fighter roster`)},'official_roster',${q(checkedAt)},CURRENT_TIMESTAMP FROM management_agencies WHERE slug=${q(agency.slug)} ON CONFLICT(agency_id,source_url) DO UPDATE SET source_title=excluded.source_title,source_type=excluded.source_type,verified_at=excluded.verified_at,last_checked_at=CURRENT_TIMESTAMP;`);
}
for(const row of matched){
  ...
  sql.push(`INSERT INTO fighter_management_evidence(management_history_id,source_url,source_title,publisher,source_type,confidence,verified_at,last_checked_at,notes) SELECT h.id,${q(row.source_url)},${q(`${row.agency_name} official fighter roster`)},${q(row.agency_name)},${q(sourceType)},${q(row.confidence)},${q(checkedAt)},CURRENT_TIMESTAMP,'Official agency roster evidence.' FROM fighter_management_history h JOIN management_agencies a ON a.id=h.agency_id WHERE h.source_key=${q(p.source_key)} AND h.source_fighter_id=${q(p.source_fighter_id)} AND h.is_current=1 AND a.slug=${q(row.agency_slug)} LIMIT 1 ON CONFLICT(management_history_id,source_url) DO UPDATE SET source_title=excluded.source_title,publisher=excluded.publisher,source_type=excluded.source_type,confidence=excluded.confidence,verified_at=excluded.verified_at,last_checked_at=CURRENT_TIMESTAMP,notes=excluded.notes;`);
