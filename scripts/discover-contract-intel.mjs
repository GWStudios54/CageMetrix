...
for(const source of CONTRACT_DISCOVERY_SOURCES){
  try{
    ...
    for(const [index,article] of articles.entries()){
      try{
        const html=await fetchText(article.url),body=articleText(html);writeFileSync(`${cache}/${source.slug}-article-${index+1}.html`,html);
...
console.log(`Contract discovery queued ${unique.length} newly observed fighter/article candidate(s); ${queue.map(row=>`${row.review_status}:${row.count}`).join(', ')} currently stored.`);
