...
SELECT p.source_key,p.snapshot_id,p.source_fighter_id,p.source_fight_id,f.event_date,f.organization,
  ...
  f.event_name,f.event_location,f.weight_class,p.result,o.source_fighter_id,o.fighter_name,NULL,f.is_title_fight,COALESCE(NULLIF(f.method_normalized,''),f.method_raw),f.round_num,f.time_finish_seconds
...
// materialize. The rating graph stays untouched: rows with an unresolved opponent do not
// retroactively change Elo, schedule strength, résumé quality or model validation.
