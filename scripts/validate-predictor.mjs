...
import { FEATURE_NAMES, PREDICTOR_VERSION, featureVector, fitLogistic, predict, modelCoefficients, sigmoid } from './lib/predictor_v01.mjs';
...
function pairedBrierInterval(rows) {
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    for (let j = 0; j < events.length; j++) {
      for (const row of events[Math.floor(random() * events.length)]) {
...
if (process.argv[1]?.endsWith('validate-predictor.mjs')) {
  ...
  const eloSlope = fitSlope(finalTrain, 'elo');
  const cmrSlope = fitSlope(finalTrain, 'cmr');

  const scored = test.map(row => ({
    ...row,
    predictor_p: predict(predictor, row),
    elo_cmr_p: predict(basic, row),
    elo_p: probability(row.elo, eloSlope),
    cmr_p: probability(row.cmr, cmrSlope)
  }));
  ...
  const report = {
    ...
    design: 'Experimental skill-based matchup predictor. Every feature is calculated from ratings built strictly before the event date. Matchups are mirrored during fitting to eliminate red/blue corner bias and force P(A,B)=1-P(B,A). Hyperparameter selection uses 2022 only; the final model is fit on 2018-2022 and evaluated from 2023 onward.',
    limitations: [
      'This is retrospective validation on the current corrected source, not a prospective record.',
      'Predictor 0.1 uses existing CMR/Elo/stat components only; age, reach, stance, injury and other context are intentionally excluded.',
      'Feature selection was specified before the 2023+ holdout, but future predictor versions must preserve this holdout discipline.',
      'Accuracy is secondary to Brier score, log loss and calibration for matchup-balance use.'
    ],
    feature_count: FEATURE_NAMES.length,
    features: FEATURE_NAMES,
    decisive_bouts_since_2018: eligible,
    matched_bouts_since_2018: rows.length,
    tuning_train_bouts: tuningTrain.length,
    tuning_validation_bouts_2022: tuningValidation.length,
    ...
    results: {
      predictor_v01: metrics(scored, 'predictor_p'),
      elo_plus_cmr: metrics(scored, 'elo_cmr_p'),
      calibrated_elo: metrics(scored, 'elo_p'),
      calibrated_cmr: metrics(scored, 'cmr_p')
    },
    established: {
      definition: 'Both fighters have at least five prior rated bouts',
      predictor_v01: metrics(established, 'predictor_p'),
      elo_plus_cmr: metrics(established, 'elo_cmr_p'),
      calibrated_elo: metrics(established, 'elo_p')
    },
    predictor_minus_elo_brier_95_interval: pairedBrierInterval(scored),
    calibration: calibration(scored, 'predictor_p'),
    top_coefficients: modelCoefficients(predictor).slice(0, 15),
    per_year: Object.fromEntries(years.map(year => {
      const subset = scored.filter(r => r.date.startsWith(year));
      return [year, { predictor_v01: metrics(subset, 'predictor_p'), calibrated_elo: metrics(subset, 'elo_p') }];
    }))
  };
