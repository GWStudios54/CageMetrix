...
import { parseDelimited, parseDate, parseHeightCm, parseReachCm, slugify } from './lib/csv.mjs';
import { buildObservations, careerAggregates, buildRatings } from './lib/model.mjs';

const DB = 'cagemetrix';
const DATASET_KEY = 'ufc-datalab-2026-06-27-v1';
...
async function main() {
  ...
  try {
    for (const [file, statements] of [['model.sql',[model]],['fighters.sql',fighterSql],['ratings.sql',ratingSql],['state.sql',[finish]]]) {
  ...
  console.log(`CageMetrix seed complete: ${fighters.length} fighters, ${ratingRows.length} ratings, data through ${sourceMaxDate}.`);
}
